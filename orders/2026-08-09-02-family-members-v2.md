...
권사님 혼자 글을 올리는 공간에서 한 단계 발전하여 가족들도 직접 묵상과 편지를 남기고, 서로의 글을 안전하게 읽는 경험을 만들기 위해서입니다.

## 사용자와 권한

- 권사님 `Admin`: 가족 글 전체 읽기·쓰기, 모든 글 삭제
...
- 권사님은 모든 글에 삭제 버튼이 보이고 실제로 모두 삭제할 수 있어야 합니다.
...
- 권사님 프로필을 `admin`으로 지정하고 가족 계정을 만듭니다.
...
Supabase SQL Editor에서 `002_family_members_and_roles.sql`을 실행하고 권사님 계정을 `admin`으로 지정해야 합니다. 실제 Admin·Member 계정별 읽기·쓰기·삭제 확인은 migration 적용 후 사용자가 브라우저에서 진행합니다.
